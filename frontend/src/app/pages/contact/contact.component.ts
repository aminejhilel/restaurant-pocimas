  ...
  template: `
    ...
    <div class="relative pt-32 pb-16 px-6">
      <div class="text-center max-w-2xl mx-auto">
        ...
        <h1 class="section-title text-5xl md:text-7xl">{{ 'CONTACT.TITLE' | translate }}</h1>
    ...
    <div class="px-6 pb-24">
      <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16">
        <!-- Info -->
        <div>
          <h2 class="font-heading text-3xl text-cream-100 mb-8">{{ 'CONTACT.VISIT' | translate }}</h2>
          <div class="space-y-6 mb-10">
            ...
            <div class="flex items-start gap-4">
              ...
              <div>
                <p class="font-accent text-xs text-bronze-400 uppercase tracking-widest mb-1">{{ 'CONTACT.EMAIL' | translate }}</p>
        ...
        <div class="card-glass p-8">
          <h2 class="font-heading text-3xl text-cream-100 mb-2">{{ 'CONTACT.FORM_TITLE' | translate }}</h2>
          <p class="font-accent text-sm text-dark-500 mb-8">{{ 'CONTACT.FORM_SUB' | translate }}</p>
          ...
          <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-5">
            <div>
              <label class="font-accent text-xs uppercase tracking-widest text-cream-400 mb-2 block">{{ 'CONTACT.NAME' | translate }}</label>
              <input formControlName="name" class="input-field" [placeholder]="'CONTACT.NAME_PH' | translate">
              <p *ngIf="f['name'].touched && f['name'].errors?.['required']" class="text-red-400 text-xs mt-1 font-accent">{{ 'CONTACT.NAME_REQ' | translate }}</p>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="font-accent text-xs uppercase tracking-widest text-cream-400 mb-2 block">{{ 'CONTACT.EMAIL_LABEL' | translate }}</label>
                <input formControlName="email" type="email" class="input-field" [placeholder]="'CONTACT.EMAIL_LABEL' | translate">
                <p *ngIf="f['email'].touched && f['email'].errors?.['required']" class="text-red-400 text-xs mt-1 font-accent">{{ 'CONTACT.EMAIL_REQ' | translate }}</p>
              </div>
              <div>
                <label class="font-accent text-xs uppercase tracking-widest text-cream-400 mb-2 block">{{ 'CONTACT.PHONE_LABEL' | translate }}</label>
                <input formControlName="phone" class="input-field" placeholder="+33 1 00 00 00 00">
              </div>
            </div>
            <div>
              <label class="font-accent text-xs uppercase tracking-widest text-cream-400 mb-2 block">{{ 'CONTACT.MESSAGE' | translate }}</label>
              <textarea formControlName="message" rows="5" class="input-field resize-none" [placeholder]="'CONTACT.MESSAGE_PH' | translate"></textarea>
              <p *ngIf="f['message'].touched && f['message'].errors?.['required']" class="text-red-400 text-xs mt-1 font-accent">{{ 'CONTACT.MESSAGE_REQ' | translate }}</p>
            </div>
            <button type="submit" [disabled]="loading"
              ...
              {{ loading ? ('CONTACT.SENDING' | translate) : ('CONTACT.SEND' | translate) }}
            </button>
          </form>
        </div>
      </div>
